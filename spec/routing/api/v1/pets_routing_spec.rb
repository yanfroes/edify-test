...
RSpec.describe Api::V1::PetsController, type: :routing do
  ...
  it { should route(:put, '/api/v1/pets/1').to(action: :update, id: 1) }
  it { should route(:patch, '/api/v1/pets/1').to(action: :update, id: 1) }
  it { should route(:delete, '/api/v1/pets/1').to(action: :destroy, id: 1) }
end
